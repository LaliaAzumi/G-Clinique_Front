...
const ChambresPage = () => {
  ...
  const [filterSoins, setFilterSoins] = useState("all");
  ...
  const filtered = chambres.filter((c) => {
    ...
    const matchEtat =
      filterEtat === "all" ||
      (filterEtat === "libre" && c.etat === true) ||
      (filterEtat === "occupee" && c.etat === false);

    const matchSoins =
      filterSoins === "all" ||
      (filterSoins === "oui" && c.isSoinsIntensifs === true) ||
      (filterSoins === "non" && c.isSoinsIntensifs === false);

    return matchSearch && matchEtat && matchSoins;
  });
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex justify-between items-center bg-black/20 backdrop-blur-md p-6 rounded-2xl border border-white/20">
          <div>
            ...
            <p className="text-white/60">Liste des chambres</p>
          </div>
  ...
  <div className="relative w-full max-w-md">
    <Search className="absolute left-2 top-1/2 -translate-y-1/2 z-10" size={18} />

    <input
      value={search}
      onChange={(e) => setSearch(e.target.value)}
      placeholder="Rechercher..."
      className="login-input pl-10 w-full placeholder-white/60"
      style={{ background: "#9797972d", color: "white" }}
    />
  </div>
  ...
  <select
    ...
    className="login-input w-[180px]"
    style={{ background: "#9797972d", color: "white" }}
  >
    <option  style={{ background: "#267a7abd", color: "white" }} value="all">Tous états</option>
    <option style={{ background: "#267a7abd", color: "white" }} value="libre">Libre</option>
    <option style={{ background: "#267a7abd", color: "white" }} value="occupee">Occupée</option>
  </select>

  {/* SOINS */}
  <select
    value={filterSoins}
    onChange={(e) => setFilterSoins(e.target.value)}
    className="login-input w-[200px]"
    style={{ background: "#9797972d", color: "white" }}
  >
    <option  style={{ background: "#267a7abd", color: "white" }} value="all">Tous soins</option>
    <option  style={{ background: "#267a7abd", color: "white" }} value="oui">Soins intensifs</option>
    <option  style={{ background: "#267a7abd", color: "white" }} value="non">Normal</option>
  </select>
            ...
            <tbody>
              {filtered.map((c) => (
                <tr key={c.id} className="hover:bg-white/5">
                  ...
                  <td className="p-4">{c.etage}</td>
                  <td className="p-4">{c.prixJ}</td>
                  <td className="p-4">{c.etat ? "Libre" : "Occupée"}</td>
                  <td className="p-4">{c.isSoinsIntensifs ? "Oui" : "Non"}</td>
